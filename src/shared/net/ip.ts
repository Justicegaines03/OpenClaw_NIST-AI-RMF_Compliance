...
type IPv4Address = {
  kind: () => "ipv4";
  range: () => string;
  match: (other: [IPv4Address, number]) => boolean;
  toString: () => string;
};
type IPv6Address = {
  kind: () => "ipv6";
  range: () => string;
  match: (other: [IPv6Address, number]) => boolean;
  parts: number[];
...
const RFC2544_BENCHMARK_PREFIX: [IPv4Address, number] = [ipaddr.IPv4.parse("198.18.0.0"), 15];
